...
// Active Voice Rooms tracking: voiceRoomId -> Array of { socketId, userId, user, isMuted, isDeafened, isCamOn, isScreenSharing }
...
io.on('connection', (socket) => {
  ...
  socket.on('voice-join', ({ voiceChannelId, user, isMuted, isDeafened, isCamOn, isScreenSharing }) => {
    ...
    // Get current peers list for joining user
    ...
    // Broadcast peer joined to existing users in room
    socket.to(voiceChannelId).emit('voice-peer-joined', memberData);

    // Broadcast active voice members update for channel list counter
  ...
  socket.on('disconnect', () => {
    ...
    if (userId) {
      ...
      // If no other sockets for this user
